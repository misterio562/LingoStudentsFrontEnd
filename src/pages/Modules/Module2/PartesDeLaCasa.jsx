...
import partesdelacasa from "./partesdelacasa.json"
...
export const PartesDeLaCasa = () => {
  ...
  const handleClick = (index) => {
    ...
    if (clickedButtons.indexOf(index) === -1) {
      ...
      if (clickedButtons.length + 1 === partesdelacasa.length) {
  ...
  return (
    <>
      <Tittle titleSpanish="Partes de la Casa" titleEnglish="House Parts" />
      <p className="text-2xl pb-3">
        Presiona cualquier parte de la casa y escucha como se dice en ingles
      </p>
      <div className="flex justify-center pb-10">
        <div className="grid grid-cols-4 grid-rows-3 gap-4 w-4/4 font-lilita">
        {[...Array(partesdelacasa.length)].map((_, index) => (
            <div
              key={index}
              onClick={() => {
                playAudio(partesdelacasa[index].pronuntiation);
                handleClick(index);
              }}
            ...
            >
              <img className="w-20" src={partesdelacasa[index].image} />
              <div>🇺🇸{partesdelacasa[index].nameInEnglish}</div>
              <div>🇪🇸{partesdelacasa[index].nameInSpanish}</div>
            </div>
          ))}
        </div>
      </div>
      {allButtonsClicked && (
        <div className="fixed inset-0 z-50 flex justify-center items-center">
          <div className="bg-white w-96 rounded-lg shadow-lg p-6">
            ...
            <p className="text-lg mb-6">¡Has escuchado las partes de la casa!.</p>
            <button
